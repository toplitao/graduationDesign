...
//            header('location:/index/index/login');
    ...
    public function confirm_repair() {
         ...
         for($i=0;$i<count($fittings_arr);$i++) {
             ...
             //配件出库记录
             $insert = ['oid' => $data['aid'], 'fid' => $fittings_data[0],'created_at' => $time,'updated_at'=>$time,'number' => $fittings_data[1],'username'=> $this->user['username'],'uid'=>$this->user['id'],'node'=>'修理需要','type'=>2];
